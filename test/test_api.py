...
def test_get_data(base_url):
    ...
    assert isinstance(response.json(), tuple)

def test_trip_data(base_url):
...
def test_nearest(base_url):
    response = requests.get(f'{base_url}/nearest', json={"lat":"30.2862730619728","long":"-97.73937727490916"})
    assert response.status_code == 200